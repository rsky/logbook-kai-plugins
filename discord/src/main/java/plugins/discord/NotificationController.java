...
public class NotificationController implements StartUp {
    ...
    private void checkNotifyMission() {
        ...
        for (DeckPort port : ports.values()) {
            ...
            if (0 == state) {
                ...
                // 残り時間を計算
                var now = Duration.ofMillis(time - currentTime);
                // 前回の通知の時間
     ...
     * @param ndock 入渠ドック
     */
    private void discordNotifyNdock(Ndock ndock) {
     ...
     */
    private void discordNotify(String title, String message) {
        var config = DiscordConfig.get();
